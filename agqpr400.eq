...
define file alt_agqvehicle = access agqvehicle,
                                set agqvehicle:app_no = agqprint1:app_no, generic

define file alt_agqmastsupp = access agqmastsupp, 
                                 set agqmastsupp:app_no           = agqvehicle:app_no,
                                     agqmastsupp:line_of_business = agqvehicle:line_of_business, generic

--define file alt_agqgaragekeepers1 = access agqgaragekeepers1,
--                                       set agqgaragekeepers1:app_no = agqvehicle:app_no, generic

define file alt_agqname = access agqname, 
                             set agqname:app_no = agqvehicle:app_no, generic

define unsigned ascii number l_line_of_business[4] = if agqname:line_of_business <> 0 then
    agqname:line_of_business
else
    agqvehicle:line_of_business

define file alt_sfsline  = access sfsline, 
...
define file sfsfob_alt = access sfsfob, set sfsfob:company_id = agqvehicle:company_id,
...
define string l_city = trun(agqname:city) + ", " + agqname:str_state
                                 
define file alt_sfsagent = access sfsagent, 
                              set sfsagent:company_id = agqvehicle:company_id,
                                  sfsagent:agent_no   = agqvehicle:agent_no, generic

define file alt_sfscomun = access sfscomun, 
                              set sfscomun:company_id = agqvehicle:company_id,
                                  sfscomun:commercial_underwriter= alt_sfsagent:commercial_underwriter, generic

define file alt_agqend1 = access agqend1, 
                             set agqend1:app_no     = agqvehicle:app_no,
                                 agqend1:vehicle_no = agqvehicle:vehicle_no, generic
  
define file alt_sfsemail = access sfsemail, 
                              set sfsemail:company_id = agqvehicle:company_id,
                                  sfsemail:user_id    = username, generic

define file alt_sfsfob = access sfsfob, 
                            set sfsfob:company_id       = agqvehicle:company_id,
                                sfsfob:form_of_business = agqmaster:form_of_business, generic
...
box/noblanklines/noheadings 
      ...
      IF alt_agqmastsupp:additional_insured = "Y" THEN
        {
          "ADDITIONAL INSUREDS - VOLUNTEERS: "
          "YES"/NEWLINE/COLUMN=50
        }
...
--        else
--            agqpanofault:medical_expense_limit
        } 
...
if agqend1:app_no = agqvehicle:app_no and
   agqend1:vehicle_no = agqvehicle:vehicle_no then 
    ...
    agqend1:premium/column=51/mask="ZZZ,ZZZ-"/newline
     
--If agqname:company_id = "LEBINS" and
--   agqend1:code = "CA9937" then
--  {
    If agqgaragekeepers1:limit <> 0 then
      {
        "Customers' Autos: "/column=1
         agqgaragekeepers1:limit/mask="ZZZ,ZZZ"/column=50/newline
      }
    If agqgaragekeepers1:comp_deductible <> 0 then
      {
        "Comprehensive Deductible: "/column=1
         agqgaragekeepers1:comp_deductible/mask="ZZZZ"/column=50/newline
      }
    If agqgaragekeepers1:comp_deductible_per_loss <> 0 then
      {
        "Maximum Deductible: "/column=1
         agqgaragekeepers1:comp_deductible_per_loss/mask="ZZZZ"/column=50/newline
      }
    If agqgaragekeepers1:coll_deductible <> 0 then
      {
        "Collision Deductible: "/column=1
         agqgaragekeepers1:coll_deductible/mask="ZZZZ"/column=50/newline
      }
    "Type: "/column=1
     If agqgaragekeepers1:specified_perils_form = 1 or
        agqgaragekeepers1:comp_form = 1 or
        agqgaragekeepers1:coll_form = 1 then
       {
         "Legal Liability"/column=51/newline
       }
     If agqgaragekeepers1:specified_perils_form = 2 or
        agqgaragekeepers1:comp_form = 2 or
        agqgaragekeepers1:coll_form = 2 then
       {
         "Direct Primary"/column=51/newline
       }
     If agqgaragekeepers1:specified_perils_form = 3 or
        agqgaragekeepers1:comp_form = 3 or
        agqgaragekeepers1:coll_form = 3 then
       {
         "Direct Coverage Excess"/column=51/newline
       }
--  }
end box/newline
...
"GARAGING ZIP CODE:"/column=6
agqvehicle:str_zipcode/mask="XXXXX-XXXX"/column=50/newline
...
"CLASS CODE"/column=6
l_class/column=50/newline/mask="99999"
"STATE"/column=6
...
if alt_agqmastsupp:employees_as_insureds = "Y" then
  {
    "Employees As Insureds:"/column=1
     "Yes"/column=30/newline
  }

if alt_agqmastsupp:nonowned_partnership  = 1 then
  {
    "Non-Owned Auto Partnership:"/column=1
    "Yes"/column=30/newline=2
  }
...
if l_surcharge <> 0 then
    {
    "SURCHARGE"/column=1
    l_surcharge/column=47/mask="Z,ZZZ,ZZZ.99-"/newline
    }
...
agqname:address[1]/newline/column=31
agqname:address[2]/newline/column=31
agqname:address[3]/newline/column=31
box/noblanklines/squeeze/column=31
l_city
agqname:str_zipcode/newline
end box
end box/newline
...
agqvehicle:business_desc[1] + agqvehicle:business_desc[2]/newline=2

"FORM OF BUSINESS:"/column=1
alt_sfsfob:description
"COVERED BY WORKERS COMP:"/column=50
agqmaster:covered_by_workers_comp/newline
"STACKED:"/column=1
switch(agqmaster:non_stacked)
  case "N" : "N"
  case "S" : "Y"
"FLEET:"/column=50
switch(agqmaster:non_Fleet)
  case "N" : "N"
  case "F" : "Y"/newline=2
